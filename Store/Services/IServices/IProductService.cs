...
namespace Store.Services.IServices
{
    ...
    {
        Task<ResponseData> GetAllAsync(int pageNumber = 1, int pageSize = 10, string? search = null, int? categoryId = null, int? sellerId = null, bool? isActive = null);
        ...
        Task<ResponseMessage> DeleteAsync(int productId, int sellerId, bool hardDelete = false);
        ...
        Task<ResponseMessage> ToggleActiveStatusAsync(int productId, int userId);
        Task<ResponseData> CheckProductUsageAsync(int productId);
    }
}
