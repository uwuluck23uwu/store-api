...
public class LocationController : ControllerBase
{
    ...
    /// Upload location image (Admin or Seller can upload their own location image)
    [HttpPost("{id:int}/upload-image")]
    [Authorize(Roles = "Admin,Seller")]
    public async Task<IActionResult> UploadLocationImage(int id, IFormFile file)
    {
        try
        {
            ...
            if (userRole == "Seller" && !string.IsNullOrEmpty(userId))
            {
                ...
                if (location == null)
                {
                    return NotFound(new { isSuccess = false, message = "ไม่พบข้อมูล Location" });
                }
                ...
                if (seller == null || location.SellerId != seller.SellerId)
                {
                    return Forbid(); // 403 Forbidden - Seller can only upload to their own location
                }
            }
